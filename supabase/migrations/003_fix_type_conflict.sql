-- Fix type conflict migration - handles the ingestion_status type/table conflict

-- Drop existing tables if they exist (in reverse dependency order)
DROP TABLE IF EXISTS undo_logs CASCADE;
DROP TABLE IF EXISTS session_proposals CASCADE;
DROP TABLE IF EXISTS metadata_raw CASCADE;
DROP TABLE IF EXISTS ingestion_status CASCADE;
DROP TABLE IF EXISTS preferences CASCADE;

-- Drop existing types if they exist
DROP TYPE IF EXISTS proposal_status CASCADE;
DROP TYPE IF EXISTS ingestion_status_enum CASCADE;
...
-- Create custom types with different names to avoid conflicts
CREATE TYPE proposal_status AS ENUM ('draft', 'applied', 'reverted');
CREATE TYPE ingestion_status_enum AS ENUM ('pending', 'processing', 'done', 'error');
...
-- Ingestion status tracking (using the enum with different name)
